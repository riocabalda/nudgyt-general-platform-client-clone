...
const Environment = () => {
  ...
  const { environmentId, setEnvironmentId } = useServiceStore()

  const {
    data: environmentData,
    isLoading,
    error
  } = useSWR(`${orgSlug}/trainer/environments`, () =>
  ...
  const selectedEnvironment =
    environmentData?.data && environmentId
  ...
  return (
    <div className='px-4 lg:px-0 pt-0 lg:py-10 max-w-[712px] mx-auto'>
      ...
      <div className='mt-[24px] flex items-center justify-between w-full'>
        ...
        <NextStepButton nextStep={3} disabled={!environmentId} />
      </div>
    </div>
  )
}
