...
function Templates() {
  ...
  const {
    ...
    isLoading,
    error
  } = useSWR(`/${orgSlug}/admin/templates?${searchParams.toString()}`, () =>
    templateService
  ...
  const transformedTemplatesData = useMemo(() => {
    return templatesData?.data.map((template: any) => ({
      ...template,
      is_shared:
        template.shared_to_organizations?.includes(
          membership?.organization._id
        ) || template.organization === membership?.organization._id
    }))
        ...
        {transformedTemplatesData.map((template: any) => (
          <li data-cy='templates' key={template._id}>
            <TemplateCard
              ...
              link={`/${orgSlug}/admin/templates/${template._id}`}
            />
          </li>
        ))}
      </ul>
