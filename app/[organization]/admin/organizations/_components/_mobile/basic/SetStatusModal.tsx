...
function SetStatusModal({ onCancel }: SetStatusModalProps) {
  ...
  const { selectedOrganizations, showMultipleCheckbox, clearOrganizations } =
    useOrganizationsStore()
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [selectedStatus, setSelectedStatus] = useState<string | null>(null)
  const [isSubmitting, setIsSubmitting] = useState(false)

  const selectedEntities = selectedOrganizations
  ...
  const handleContinue = async () => {
    ...
    try {
      const organizationIds = selectedOrganizations.map((org) => org._id)
  ...
  const showButton = showMultipleCheckbox && selectedOrganizations.length > 0
