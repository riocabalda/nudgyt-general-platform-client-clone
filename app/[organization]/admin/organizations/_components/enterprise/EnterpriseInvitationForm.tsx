...
type EnterpriseInvitationFormData = z.infer<
  typeof EnterpriseInvitationFormSchema
>

const EnterpriseInvitationFormSchema = z.object({
  ...
  platformUrl: z
    ...
    .min(1, 'Please enter a platform URL')
})

function EnterpriseInvitationForm(props: { onSubmitSuccess?: () => void }) {
  const { onSubmitSuccess } = props
  const { orgSlug } = useOrganization()
  const form = useForm<EnterpriseInvitationFormData>({
    resolver: zodResolver(EnterpriseInvitationFormSchema),
    defaultValues: {
      monthlyAmount: 0,
      userSeats: 0,
      organizationName: '',
  ...
  async function sendInvite(formData: EnterpriseInvitationFormData) {
    setIsSubmitting(true)
    try {
      await userService.inviteOwnerEnterprise(orgSlug, formData)
    ...
    <FormProvider {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>User Seats</FormLabel>
              <FormControl>
                <Input
                  {...field}
                  type='number'
        ...
        {globalErrorMessage !== undefined && (
          <p className='font-medium text-destructive'>{globalErrorMessage}</p>
        )}
...
export default EnterpriseInvitationForm
