...
function interceptSimulationResults(simulationId: string) {
  cy.intercept(
    'GET',
    `${BACKEND_URL}/system/admin/simulation/${simulationId}/results`,
    (req) => {
      req.reply({
        statusCode: 200,
...
function authenticateUser(role: Role) {
  cy.intercept('GET', `${BACKEND_URL}/auth/me`, {
    fixture: `auth/me/success/system/${role}.json`
  })
}
...
function testSimulationResultsOnServiceDetailsTab() {
  it('Should display simulation results on the service details tab', () => {
    ...
    cy.visit('/system/admin/simulation/1/results')

    authenticateUser(role)
    authenticateUser(role)

    interceptSimulationResults('1')
    interceptAudioFile()
    cy.get('h1')
      .contains(mockSimulationResults.serviceData.title)
      .should('be.visible')
    cy.get('h3')
      .contains(mockSimulationResults.learner.fullname)
      .should('be.visible')
    cy.get('div')
      .contains(mockSimulationResults.learner.user_type)
      .should('be.visible')
