...
import loggedInAdmin from '../../../fixtures/auth/me/success/admin.json'
  ...
  DEVICES.forEach((device) => {
    context(device, () => {
      ...
      it('should display user services', () => {
        ...
        cy.intercept(
          'GET',
          `${apiBaseUrl}/${orgSlug}/admin/services?user_services=${userId}`,
          {
            statusCode: 200,
            body: {
      ...
      it('should handle no results found in user services', () => {
        ...
        cy.intercept(
          'GET',
          `${apiBaseUrl}/${orgSlug}/admin/services?user_services=*`,
          {
            statusCode: 200,
            body: {
              data: mockServices.data.filter(
